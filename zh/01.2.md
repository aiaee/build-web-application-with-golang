...
cd ~/gopher/gomodtest
go mod init github.com/filePath/gomodtest
...
如果未初始化go mod init 会出现错误 go.mod file not found in current directory or any parent directory; see ‘go help modules’
是因为未启用的gomodle管理，输入 go env 发现set GO111MODULE= 输入命令 go env -w GO111MODULE=off 再次运行即可
